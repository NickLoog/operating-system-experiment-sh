use crate::sync::UPSafeCell; // 假设 UPSafeCell 在 sync 模块中
use lazy_static::*;
...
struct AppManager {
    ...
    current_app: usize,                           // 当前正在运行的应用 ID
...
lazy_static! {
    static ref APP_MANAGER: UPSafeCell<AppManager> = unsafe {
        UPSafeCell::new({
            ...
            AppManager {
                num_app,
                current_app: 0, // 初始时没有应用在运行，设为0准备加载第一个
                app_start,
            }
        })
    };
...
impl AppManager {
    ...
    unsafe fn load_app(&self, app_id: usize) {
        ...
        // 清空应用程序将要加载到的内存区域 (0x80400000 开始)
        core::slice::from_raw_parts_mut(APP_BASE_ADDRESS as *mut u8, APP_SIZE_LIMIT).fill(0);

        // 获取应用程序在内核镜像中的二进制数据切片
        let app_src = core::slice::from_raw_parts(
            self.app_start[app_id] as *const u8,                   // 起始地址
            self.app_start[app_id + 1] - self.app_start[app_id], // 长度 = 结束地址 - 起始地址
        );

        // 获取应用程序将要加载到的目标内存区域的切片
        let app_dst = core::slice::from_raw_parts_mut(APP_BASE_ADDRESS as *mut u8, app_src.len());

        // 将应用程序二进制数据从内核镜像复制到目标内存区域
        app_dst.copy_from_slice(app_src);
        println!("[kernel] App_{} loaded.", app_id);
    }
...
pub fn run_next_app() -> ! {
    let mut app_manager = APP_MANAGER.exclusive_access(); // 获取 AppManager 的可变引用
    ...
    panic!("App loaded but execution not implemented yet!");
}
// --- End 公共接口 ---
